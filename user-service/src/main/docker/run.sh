...
#echo "********************************************************"
#while ! `nc -z database-user $DATABASESERVER_PORT`; do sleep 3; done
...
echo "********************************************************"
java -Xmx256m -Djava.security.egd=file:/dev/./urandom -Dserver.port=$SERVER_PORT   \
     ...
     -jar /usr/local/service/@project.build.finalName@.jar
